export const augmentTable = (table, html, data) => {
  ...
  const flag = table.object.getFlag("adarkdeep", "treasure");
  const treasure = flag
  ...
  html.find(".toggle-treasure").click((ev) => {
    let isTreasure = table.object.getFlag("adarkdeep", "treasure");
    table.object.setFlag("adarkdeep", "treasure", !isTreasure);
  });
  ...
  if (flag) {
    ...
    // Replace Roll button
    const roll = `<button class="roll-treasure" type="button"><i class="fas fa-gem"></i> ${game.i18n.localize('ADARKDEEP.table.treasure.roll')}</button>`;
...
function drawTreasure(table, data) {
  ...
  data.treasure = {};
  if (table.getFlag('adarkdeep', 'treasure')) {
...
async function rollTreasure(table, options = {}) {
  ...
  let html = await renderTemplate(
    "systems/adarkdeep/templates/chat/roll-treasure.html",
    templateData
  );

  let chatData = {
    content: html,
    // sound: "/systems/adarkdeep/assets/coins.mp3"
  }
