...
//    }
  ...
  static async sendAttackRoll({
    ...
    const template = "systems/adarkdeep/templates/chat/roll-attack.html";
  ...
  static async RollSave({
    ...
    let rolled = false;
    const template = "systems/adarkdeep/templates/chat/roll-dialog.html";
    let dialogData = {
      formula: parts.join(" "),
      data: data,
    ...
    let buttons = {
      ok: {
        label: game.i18n.localize("ADARKDEEP.Roll"),
      ...
      magic: {
        label: game.i18n.localize("ADARKDEEP.saves.magic.short"),
        icon: '<i class="fas fa-magic"></i>',
        callback: (html) => {
          rolled = true;
          rollData.form = html[0].querySelector("form");
          rollData.parts.push(`${rollData.data.roll.magic}`);
          rollData.title += ` ${game.i18n.localize("ADARKDEEP.saves.magic.short")} (${rollData.data.roll.magic})`;
          roll = OseDice.sendRoll(rollData);
        },
      },
      cancel: {
