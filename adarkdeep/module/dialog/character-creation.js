...
export class OseCharacterCreator extends FormApplication {
  static get defaultOptions() {
    ...
    (options.classes = ["adarkdeep", "dialog", "creator"]),
      (options.id = "character-creator");
    options.template =
      "systems/adarkdeep/templates/actors/dialogs/character-creation.html";
    options.width = 235;
    return options;
  }
   ...
   */
  get title() {
    return `${this.object.name}: ${game.i18n.localize("ADARKDEEP.dialog.generator")}`;
  }
  ...
  rollScore(score, options = {}) {
    ...
    const label =
      score != "gold" ? game.i18n.localize(`ADARKDEEP.scores.${score}.long`) : "Gold";
    const rollParts = ["3d6"];
    const data = {
      roll: {
        type: "result",
    ...
    return OseDice.Roll({
      ...
      flavor: game.i18n.format("ADARKDEEP.dialog.generateScore", {
        score: label,
        count: this.counters[score],
      }),
      title: game.i18n.format("ADARKDEEP.dialog.generateScore", {
        score: label,
        count: this.counters[score],
      }),
    });
  ...
  async close(options) {
    ...
    const templateData = {
      ...
      scores: this.scores,
      title: game.i18n.localize("ADARKDEEP.dialog.generator"),
      stats: this.object.data.stats,
      gold: this.gold,
    };
    const content = await renderTemplate(
      "/systems/adarkdeep/templates/chat/roll-creation.html",
      templateData
    );
    ChatMessage.create({
      content: content,
  ...
  async _onSubmit(
    ...
    const itemData = {
      name: "GP",
      type: "item",
      img: "/systems/adarkdeep/assets/gold.png",
      data: {
        treasure: true,
        cost: 1,
        weight: 1,
